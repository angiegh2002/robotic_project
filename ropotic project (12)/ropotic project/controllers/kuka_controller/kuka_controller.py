...
class YouBotController(Robot):
    ...
    def pick_object(self):
        ...
        self.lower_arm()        # خفض الذراع
        self.step(1000)         # انتظار
        self.open_gripper()     # فتح القابض
        self.step(1500)         # انتظار  
        self.close_gripper()    # إغلاق القابض
        self.step(1000)         # انتظار
        self.lift_arm()         # رفع الذراع
        self.step(100)         # انتظار
    ...
    def rotate_180_degrees(self):
        ...
        self.front_left_wheel.setVelocity(14.0)
        self.back_left_wheel.setVelocity(14.0)
        self.step(1700)  # ضعف وقت الدوران 90 درجة
        ...
        while self.step(self.timestep) != -1:
            ...
            if not returning_to_main:
                if not has_cube:
                    ...
                    if current_color == "yellow" and r == 1:
                        self.step(100)
                        self.turn_right()
                        while self.step(self.timestep) != -1:
                            front_distance = self.detect_object()
                    ...
                    elif current_color == "green" and l == 1:
                        self.step(100)
                        self.turn_left()
                        while self.step(self.timestep) != -1:
                            front_distance = self.detect_object()
                    ...
                    elif current_color == "blue" and l == 1:
                ...
                else:  # إذا كان يحمل المكعب
                    self.follow_line()
                    r, l = self.handle_distance_and_color()
                    if current_color == "green" or current_color == "blue":
                        if r == 1:
                            self.step(300)
                            self.turn_right()
                ...
                self.follow_line()
                front_distance1 = self.detect_object()
                if front_distance1 == 1:  # وصل إلى الحائط
                    self.stop()
                    if current_color == "green" or current_color == "blue" or current_color == "yellow":
                        self.front_right_wheel.setVelocity(self.v)
                        self.back_right_wheel.setVelocity(-self.v)
                        self.front_left_wheel.setVelocity(-self.v)
                        self.back_left_wheel.setVelocity(self.v)
